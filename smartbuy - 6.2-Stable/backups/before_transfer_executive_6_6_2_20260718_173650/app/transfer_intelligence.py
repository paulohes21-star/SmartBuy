...
class TransferRecommendation:
    ...
    priority: str
    explanation: str
    origin_after_stock: float
    destination_after_stock: float
    origin_after_coverage_days: float | None
    destination_after_coverage_days: float | None
...
def calculate_transfers(
    ...
    for row in enriched:
        ...
        if consumption > 0:
            assessed_rows += 1
            if 60 <= float(coverage or 0) <= excess_days:
                healthy_rows += 1
            excess_value_qty = max(stock - max(safety, consumption * excess_days), 0)
            idle_capital += excess_value_qty * unit_cost
        elif stock > safety:
            assessed_rows += 1
            no_turnover_value += max(stock - safety, 0) * unit_cost
            idle_capital += max(stock - safety, 0) * unit_cost
    ...
    for product_rows in by_product.values():
        ...
        for destination in destinations:
            ...
            for origin in origins:
                ...
                origin_after_stock = float(origin["available_stock"]) - quantity
                destination_after_stock = float(destination["available_stock"]) + quantity
                origin_after_coverage = (
                    origin_after_stock / float(origin["daily_consumption"])
                    if float(origin["daily_consumption"] or 0) > 0 else None
                )
                destination_after_coverage = (
                    destination_after_stock / float(destination["daily_consumption"])
                    if float(destination["daily_consumption"] or 0) > 0 else None
                )
                ...
                recommendations.append(TransferRecommendation(
                    ...
                    origin_after_stock=round(origin_after_stock, 2),
                    destination_after_stock=round(destination_after_stock, 2),
                    origin_after_coverage_days=origin_after_coverage,
                    destination_after_coverage_days=destination_after_coverage,
                ))
    ...
    health_score = round((healthy_rows / assessed_rows) * 100) if assessed_rows else 100
    metrics = {
        ...
        "money_idle_capital": _money(idle_capital),
        "no_turnover_value": no_turnover_value,
        "money_no_turnover_value": _money(no_turnover_value),
